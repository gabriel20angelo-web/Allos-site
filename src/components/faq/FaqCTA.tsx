    ...
    <section ref={ref} className="py-24 md:py-32 px-6 md:px-10 relative overflow-hidden" style={{ background: "#1A1A1A" }}>
      ...
      <div className="relative z-10 max-w-[700px] mx-auto text-center">
        <motion.div initial={{ opacity: 0, scaleX: 0 }} animate={inView ? { opacity: 1, scaleX: 1 } : {}}
          transition={{ duration: 0.6 }} className="flex items-center justify-center gap-3 mb-8">
        ...
        <motion.h2 initial={{ opacity: 0, y: 22 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          Não encontrou sua <span className="italic text-[#C84B31]">dúvida?</span>
        </motion.h2>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.92 }} animate={inView ? { opacity: 1, scale: 1 } : {}}
          transition={{ delay: 0.3, duration: 0.5 }} className="flex flex-wrap items-center justify-center gap-4">
          <motion.a href="https://chat.whatsapp.com/KP2z0vFRaSVBSXRjIyvR3R" target="_blank" rel="noopener noreferrer"
            whileHover={{ scale: 1.04, boxShadow: "0 6px 22px rgba(37,211,102,.3)" }} whileTap={{ scale: 0.97 }}
          ...
          <motion.a href="mailto:suporte@allos.org.br" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
            className="inline-flex items-center gap-2 font-dm font-medium rounded-full transition-all"
            style={{ color: "#C84B31", border: "1px solid rgba(200,75,49,0.4)", padding: "13px 26px", fontSize: "14px" }}>
