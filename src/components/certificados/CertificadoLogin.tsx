...
export default function CertificadoLogin({ onSuccess }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        sessionStorage.setItem('cert_admin', 'true')
        onSuccess()
      } else {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden" style={{ background: 'linear-gradient(145deg, #1A1A1A 0%, #111111 40%, #1A1714 100%)' }}>
      <div className="absolute" style={{ top: '-20%', left: '30%', width: '50%', height: '50%', background: 'radial-gradient(ellipse, rgba(200,75,49,0.08) 0%, transparent 55%)', filter: 'blur(60px)' }} />
        ...
        <div className="rounded-3xl p-8" style={{ backgroundColor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', backdropFilter: 'blur(12px)' }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button type="submit" disabled={loading}
              ...
              style={{ backgroundColor: loading ? 'rgba(253,251,247,0.1)' : '#C84B31', color: loading ? 'rgba(253,251,247,0.3)' : '#fff', boxShadow: loading ? 'none' : '0 4px 20px rgba(200,75,49,0.25)' }}>
              {loading ? 'Verificando...' : 'Entrar'}
            </button>
          </form>
        </div>
        <div className="flex items-center justify-center mt-6 gap-2">
          <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: '#C84B31' }} />
